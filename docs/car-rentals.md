...
1. Sign up, verify the account email, and open the car rental host application from the profile. Provide identity, ownership/authorization, registration, the host's insurance covering paid rentals through 6ixo (including delivery if offered), safety/support agreements, and vehicle photos. Uploads remain private; incomplete drafts do not reach admin review.
...
4. **Admin → Car rental operations** lists vehicle drafts. Review the host's policy documents or insurer confirmation covering paid rentals of this specific vehicle through 6ixo, including delivery if offered. Record coverage expiry and a guest-facing summary of the insurer, covered uses, limits, deductible, exclusions and driver eligibility. Review minimum driver age, time zone, default pickup/return times, included distance, extra-distance rate, delivery fee, pickup/return instructions, support contact and applicable taxes. Explicitly enable bookings after review. The host can then publish from their rental listings.
...
The current operating model, confirmed September 14, 2026, requires hosts to provide their own rental insurance. 6ixo's business insurance is being arranged; 6ixo does not currently provide vehicle-rental insurance or a protection plan. Host application, booking and trip screens disclose this. A personal vehicle policy or a policy covering rentals on another marketplace is not treated as proof of coverage on 6ixo. Approval requires evidence that the host's insurer covers paid rentals through 6ixo and delivery if offered. The existing admin review and coverage-expiry booking checks remain required. Host insurance does not establish that 6ixo's separate business or platform insurance obligations are satisfied. Driver approval is a documented human review, not an automated identity/background-check service.
