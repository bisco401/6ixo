...
reconnect.localGeocodeStatus = 'TIMEOUT'; reconnect.localGeocodeRetryAt = now + 60000;
...
assert.equal(reconnect.localGeocodeRetryAt, 0);
...
reconnect.localGeocodeStatus = 'OVER_QUERY_LIMIT'; reconnect.localGeocodeRetryAt = now + 60000;
windowEvents.get('online')({ type: 'online' }); await flush();
assert.equal(reconnect.localGeocodeRetryAt, now + 60000, 'Reconnect must not bypass quota cooldown');
