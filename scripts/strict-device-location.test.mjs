...
assert.equal(scope.city,'');
assert.equal(app.matchesListingLocationScope(local,scope),true);
assert.equal(app.matchesListingLocationScope({city:'Toronto',country:'Canada'},scope),true,'Other cities in the detected country remain visible');
assert.equal(app.matchesListingLocationScope(foreign,scope),false);
assert.equal(app.matchesListingLocationScope({city:'Oakville East',country:'Canada'},scope),true,'Country browsing includes all cities without changing the detected city label');
...
assert.deepEqual(featured.map(c=>c.excluded),[false,false,true,true]);
...
assert.equal(app.getHomeTodayDeals().length,2);
let finishLookup;
...
console.log('Strict device location passed: first paint, stored-profile isolation, country-wide filters/deals/featured cards, movement, render ordering, expiration, denial and recovery.');
...
assert.equal(boundaryApp.getDeviceListingLocationScope().city, '');
assert.equal(boundaryApp.resolvedDeviceLocation.city, 'North York');
